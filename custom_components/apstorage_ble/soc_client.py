...
# Write operations execute multi-step flows (session setup + one or more
...
SYSTEM_MODE_WRITE_RETRY_BACKOFF_SECONDS = (10.0, 15.0, 20.0)
        ...
        *,
        ...
        try:
            ...
            async with asyncio.timeout(CONNECT_TIMEOUT_SECONDS):
                ...
                # Use device_name_hint when available to avoid a GATT char read
                # that stalls for 30 s (bleak default timeout) when the proxy is
                # slow, by which time the connection state is fragile.
                if device_name_hint:
                    device_name = device_name_hint
                else:
                    try:
